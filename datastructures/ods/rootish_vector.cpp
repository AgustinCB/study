...
template <typename T>
inline unsigned int RootishVector<T>::size() const {
	return _size;
}

template <typename T>
inline unsigned int RootishVector<T>::capacity() const {
...
template <typename T>
const T& RootishVector<T>::get(unsigned int i) const {
	assert(i < _size);
...
template <typename T>
T& RootishVector<T>::get_mut(unsigned int i) {
	assert(i < _size);
	unsigned int block = block_for_index(i);
	unsigned int block_index = index_within_block(i, block);
...
template <typename T>
T RootishVector<T>::set(unsigned int i, T x) {
	assert(i < _size);
	T prev = get(i);
	get_mut(i) = x;
	return prev;
...
template <typename T>
void RootishVector<T>::add(unsigned int i, T x) {
...
template <typename T>
T RootishVector<T>::remove(unsigned int i) {
	T x = get(i);
...
template <typename T>
inline unsigned int RootishVector<T>::index_within_block(unsigned int index, unsigned int block) const {
	return index - blocks_capacity(block);
}

template <typename T>
inline unsigned int RootishVector<T>::block_for_index(unsigned int index) const {
	return (unsigned int) ceil(
		(-3.0 + sqrt(9 + 8 * index)) / 2.0
	);
}
